...
describe User, :vcr do
  ...
  describe "associations" do
    ...
    describe "collaborator associations" do
      ...
      it { is_expected.to have_many(:alive_collaborator_accounts).with_foreign_key(:affiliate_user_id).class_name("Collaborator") }
      it { is_expected.to have_many(:collaborating_products).through(:alive_collaborator_accounts).source(:products) }

      it "has many alive_collaborator_accounts" do
        user = create(:user)
        live_collaborators = [
          create(:collaborator, affiliate_user: user),
          create(:collaborator, affiliate_user: user),
        ]
        create(:collaborator, affiliate_user: user, deleted_at: 1.day.ago)

        expect(user.alive_collaborator_accounts).to match_array live_collaborators
      end
    end
