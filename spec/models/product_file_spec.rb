...
describe ProductFile do
  ...
  describe "file group and analyze" do
    it "enqueues analyze after file creation" do
      product_file = create(:readable_document)
      expect(AnalyzeFileWorker).to have_enqueued_sidekiq_job(kind_of(Integer))
  ...
  describe "product file for installments" do
    it "schedules analyze and not schedule PdfUnstampableNotifierJob if the file belongs to an installment" do
      create(:readable_document, link: nil, installment: create(:installment))
      expect(AnalyzeFileWorker).to have_enqueued_sidekiq_job(kind_of(Integer))
  ...
  describe "#transcoding_in_progress?" do
    it "returns true if there's a transcoded video in progress" do
      ...
      create(:transcoded_video, product_file: video_file, original_video_key: video_file.s3_key, state: "processing")
  ...
  describe "#attempt_to_transcode?" do
    ...
    it "returns `false` if there are transcoding jobs in the 'processing' state" do
      create(:transcoded_video, product_file:, original_video_key: product_file.s3_key, state: "processing")
    ...
    it "returns `false` if there are transcoding jobs in the 'completed' state" do
      create(:transcoded_video, product_file:, original_video_key: product_file.s3_key, state: "completed")
  ...
  describe "#hls_playlist" do
    before do
      ...
      @transcoded_video = create(:transcoded_video, link: @multifile_product, product_file: @file_1, original_video_key: @file_1.s3_key,
    ...
    it "escapes the filename in legacy S3 attachments" do
      ...
      @transcoded_video = create(:transcoded_video, link: @multifile_product, product_file: file, original_video_key: file.s3_key,
