...
describe Link, :vcr do
  ...
  describe "associations" do
    ...
    describe "#collaborators" do
      it "returns all product collaborators" do
        product = create(:product)
        collaborators = [
          create(:collaborator, products: [product], deleted_at: 1.minute.ago),
          create(:collaborator, products: [product]),
        ]
        expect(product.collaborators).to match_array collaborators
      end
    end
  ...
  describe "scopes" do
    ...
    describe "for_collaborating_user" do
      it "returns products that the user is a collaborator on" do
        ...
        expect(Link.for_collaborating_user(user).pluck(:id)).to match_array(seller_collabs.pluck(:id))
      end
    end

    describe "collab_for_user" do
      it "returns products that the user is a collaborator on and collabs they've created" do
        ...
        expect(Link.collab_for_user(user).pluck(:id)).to match_array(collab_ids)
      end
    end
  ...
  describe "#has_another_collaborator?" do
    let(:product) { create(:product) }
    let!(:inactive_collaborator) { create(:collaborator, products: [product], deleted_at: 1.hour.ago) }
    ...
    it "returns false if the product has no live collaborators" do
      expect(product.has_another_collaborator?).to eq false
    end

    it "returns true if the product has a live collaborator" do
      inactive_collaborator.mark_undeleted!
    ...
    context "with a collaborator" do
      ...
      context "that is a collaborator for the product" do
        before { create(:product_affiliate, affiliate: collaborator, product:) }

        it "returns false if there are no other live collaborators" do
          expect(product.has_another_collaborator?(collaborator:)).to eq false
        end

        it "returns true if the product has another live collaborator" do
          inactive_collaborator.mark_undeleted!
          expect(product.has_another_collaborator?(collaborator:)).to eq true
        end
      end

      context "that is not a collaborator for the product" do
        it "returns false if there are no other live collaborators" do
          expect(product.has_another_collaborator?(collaborator:)).to eq false
        end
  ...
  describe "#percentage_revenue_cut_for_user" do
    ...
    context "for a collab" do
      ...
      it "returns the collaborator's cut for the collaborator" do
        expect(product.percentage_revenue_cut_for_user(product.collaborator.affiliate_user)).to eq(45)
      end

      it "returns 100 less the collaborator's cut for the seller" do
        expect(product.percentage_revenue_cut_for_user(product.user)).to eq(55)
      end
