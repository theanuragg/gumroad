...
describe DirectAffiliate do
  ...
  describe "#update_posts_subscription" do
    it "updates affiliate user's all affiliate records for the creator as per send_posts parameter" do
      direct_affiliate_2 = create(:direct_affiliate, affiliate_user:, seller:, archived_at: Time.current)
      direct_affiliate_3 = create(:direct_affiliate, affiliate_user:, seller:, deleted_at: Time.current)
      ...
      expect(direct_affiliate_3.reload.send_posts).to be false
      ...
      expect(direct_affiliate_3.reload.send_posts).to be true
    end
  end
  ...
  describe "#eligible_for_purchase_credit?" do
    ...
    it "returns false if the affiliate is archived" do
      affiliate = create(:direct_affiliate, archived_at: 1.day.ago, products: [product])
      expect(affiliate.eligible_for_purchase_credit?(product:)).to eq false
    end
  ...
  context "AudienceMember" do
    ...
    describe "#should_be_audience_member?" do
      it "only returns true for expected cases" do
        ...
        affiliate = create(:direct_affiliate, archived_at: Time.current)
        expect(affiliate.should_be_audience_member?).to eq(false)
