...
describe Affiliate do
  describe "scopes" do
    describe ".alive" do
      it "excludes deleted and archived affiliates" do
        direct_affiliate = create(:direct_affiliate)
        create(:direct_affiliate, deleted_at: 1.hour.ago)
        create(:direct_affiliate, archived_at: 1.hour.ago)
        archived_global_affiliate = create(:user).global_affiliate
        archived_global_affiliate.update!(archived_at: 1.hour.ago)

        expect(described_class.alive).to match_array(GlobalAffiliate.where.not(id: archived_global_affiliate.id).to_a << direct_affiliate)
      end
    end
    ...
    describe "affiliate types" do
      ...
      describe ".direct" do
        it "returns all direct affiliates" do
          expect(Affiliate.direct).to match_array [direct_affiliate]
        end
      end

      describe ".global" do
        it "returns all global affiliates" do
          expect(Affiliate.global).to match_array GlobalAffiliate.all # we generate a `GlobalAffiliate` for every user
        end
      end

      describe ".collaborator" do
        it "returns all collaborators" do
          expect(Affiliate.collaborator).to match_array [collaborator]
        end
      end

      describe ".non_collaborator" do
        it "returns all non collaborators" do
          expect(Affiliate.non_collaborator).to match_array(GlobalAffiliate.all.to_a << direct_affiliate)
        end
      end

      describe ".non_global" do
        it "returns all non global affiliates" do
          expect(Affiliate.non_global).to match_array [direct_affiliate, collaborator]
        end
      end
    end
  ...
  describe "validations" do
    context "uniqueness of affiliate user scoped to seller" do
      ...
      it "validates the affiliate with the same scope when the existing affiliate is archived" do
        existing = create(:direct_affiliate, archived_at: 1.day.ago)
        affiliate = build(:direct_affiliate, affiliate_user: existing.affiliate_user, seller: existing.seller)
        expect(affiliate).to be_valid
      end
    end
  ...
  describe "#alive?" do
    ...
    it "returns true if not deleted" do
      expect(affiliate.alive?).to eq true
    end

    it "returns false if affiliate is deleted" do
      affiliate.deleted_at = Time.current
      expect(affiliate.alive?).to eq false
    end

    it "returns false if affiliate is archived" do
      affiliate.archived_at = Time.current
      expect(affiliate.alive?).to eq false
    end
  end
  ...
  describe "#archived?" do
    ...
    it "returns true if archived" do
      affiliate.archived_at = Time.current
      expect(affiliate.archived?).to eq true
    end

    it "returns false if not archived" do
      expect(affiliate.archived?).to eq false
    end
  end
  ...
  describe "#eligible_for_credit?" do
    ...
    it "returns false if affiliate is deleted or archived" do
