...
describe("Payments Settings Scenario", type: :feature, js: true) do
  describe "PayPal section" do
    ...
    it "disables the PayPal Connect button and shows the notification when user does not have either bank account set up" do
      creator = create(:user, payment_address: nil)
      create(:user_compliance_info, user: creator)
      login_as creator
      visit settings_payments_path
      expect(page).to have_alert(text: "You must set up credit card purchases above before enabling customers to pay with PayPal.")
      expect(page).to have_link(text: "Connect with Paypal", inert: true)
    end

    it "keeps the PayPal Connect button enabled and does not show the notification when user has stripe account connected" do
      creator = create(:user, payment_address: nil)
      create(:user_compliance_info, user: creator)
    ...
    it "keeps the PayPal Connect button enabled and does not show the notification when user has bank account connected" do
      creator = create(:user, payment_address: nil)
      create(:user_compliance_info, user: creator)
    ...
    it "keeps the PayPal Connect button enabled and does not show the notification when user has debit card connected" do
      creator = create(:user, payment_address: nil)
      create(:user_compliance_info, user: creator)
  ...
  describe("Payout Information Collection", type: :feature, js: true) do
    ...
    describe "US-based creator with information set" do
      ...
      xit "allows the creator to provide their SSN when required" do
        ...
        expect(stripe_person.verification.status).to eq("pending")
        ...
        login_as user
        visit settings_payments_path
        expect(page).to have_text("We need some more information from you before your next payout")

        fill_in("Your full social security number", with: "0")
        click_on("Submit")
        expect(find("fieldset.danger")).to have_field("Your full social security number")

        fill_in("Your full social security number", with: "123456789")
        expect(page).to have_field("Your full social security number", with: "123-45-6789") # formats SSN correctly
        fill_in("Your full social security number", with: "000000000") # use Stripe test SSN for submission
        click_on("Submit")

        expect(page).to have_content("Thanks! You're all set")
        expect(page).not_to have_text("We need some more information from you")
        expect(user.user_compliance_info_requests.requested.
            where(field_needed: UserComplianceInfoFields::Individual::TAX_ID).count).to eq(0)

        stripe_person = Stripe::Account.list_persons(merchant_account.charge_processor_merchant_id)["data"].last
        expect(stripe_person.verification.status).to eq("verified")
      end

      it "allows the creator to upload their identity document scan and updates it on stripe connect account" do
        user = create(:user, username: nil, payment_address: nil)
        create(:user_compliance_info, user:, birthday: Date.new(1901, 1, 2))
        create(:ach_account_stripe_succeed, user:)
        create(:tos_agreement, user:)
        ...
        expect(page).to have_text("Having trouble? See detailed requirements here.")
        expect(page).to have_link("here", href: "https://docs.stripe.com/acceptable-verification-documents?document-type=identity#select-a-country-to-view-its-requirements")

        # Test large file validation:
        Tempfile.new.tap do |file|
          file.write("1" * 10.megabytes)
          file.rewind

          page.attach_file("Upload image", file.path, visible: false)
          expect(page).to have_content "File can't be larger than 10 MB"
          file.close!
        end

        page.attach_file("Upload image", Rails.root.join("spec/support/fixtures/success.png"), visible: false)
        expect(page).to have_content("Uploading...")
        expect(page).to have_content("success.png")
        expect(page).to have_content("Thanks! You're all set.")
        expect(page).not_to have_text("Requirements")
      ...
      context "when the Stripe Connect embedded notification banner is enabled" do
        let(:user) { create(:user, username: nil, payment_address: nil) }

        before { Feature.activate_user(:stripe_connect_embedded_notification_banner, user) }
      ...
      it "allows the creator to upload their address-proof document scan and updates it on stripe connect account" do
        user = create(:user, username: nil, payment_address: nil)
        create(:user_compliance_info, user:, birthday: Date.new(1901, 1, 2))
        create(:ach_account_stripe_succeed, user:)
        create(:tos_agreement, user:)
        ...
        create(:user_compliance_info_request, user:, field_needed: UserComplianceInfoFields::Individual::STRIPE_ADDITIONAL_DOCUMENT_ID)
        expect(user.user_compliance_info_requests.requested.
            where(field_needed: UserComplianceInfoFields::Individual::STRIPE_ADDITIONAL_DOCUMENT_ID).count).to eq(1)

        login_as user
        visit settings_payments_path
        expect(page).to have_text("Address verification document")
        expect(page).to have_text("Requirements")
        expect(page).to have_text("Having trouble? See detailed requirements here.")
        expect(page).to have_link("here", href: "https://docs.stripe.com/acceptable-verification-documents?document-type=address#select-a-country-to-view-its-requirements")

        page.attach_file("Upload photo or scan", Rails.root.join("spec/support/fixtures/success.png"), visible: false)
        expect(page).to have_content("Uploading")
        expect(page).to have_content("success.png")
        expect(page).to have_content("Thanks! You're all set.")
        expect(page).not_to have_text("Address verification document")

        stripe_person = Stripe::Account.list_persons(merchant_account.charge_processor_merchant_id)["data"].last
        expect(stripe_person.verification.additional_document.front).to match(/^file_/)
        expect(stripe_person.verification.additional_document.back).to be(nil)
        expect(user.user_compliance_info_requests.requested.
            where(field_needed: UserComplianceInfoFields::Individual::STRIPE_ADDITIONAL_DOCUMENT_ID).count).to eq(0)
        expect(user.alive_user_compliance_info.stripe_additional_document_id).to match(/^file_/)
      end

      it "displays the verification error returned by Stripe along with the verification prompt" do
        user = create(:user, username: nil, payment_address: nil)
        create(:user_compliance_info, user:, birthday: Date.new(1901, 1, 2))
        create(:ach_account_stripe_succeed, user:)
        create(:tos_agreement, user:)
        ...
        create(:user_compliance_info_request, user:, field_needed: UserComplianceInfoFields::Individual::STRIPE_IDENTITY_DOCUMENT_ID,
                                              verification_error: { code: "verification_document_manipulated" })
        expect(user.user_compliance_info_requests.requested.
          where(field_needed: UserComplianceInfoFields::Individual::STRIPE_IDENTITY_DOCUMENT_ID).count).to eq(1)

        login_as user
        visit settings_payments_path
        expect(page).not_to have_text("We need a government-issued photo ID")
        expect(page).to have_text("The document might have been altered so it could not be verified.")
        expect(page).to have_text("Requirements")
        ...
        expect(page).not_to have_text("We need a government-issued photo ID")
        expect(page).not_to have_text("Requirements")
        expect(page).not_to have_text("The document might have been altered so it could not be verified.")
      ...
      it "does not show the verification section if Stripe account is not active" do
        ...
        visit settings_payments_path
        expect(page).to have_text("We need some more information from you before your next payout")
        expect(page).to have_text("Your full social security number")

        merchant_account.mark_deleted!
        visit settings_payments_path
        expect(page).not_to have_text("We need some more information from you before your next payout")
        expect(page).not_to have_text("Your full social security number")
      end
      ...
      context "when the creator has a business account" do
        ...
        it "allows the creator to upload their company document scan and updates it on stripe connect account" do
          visit settings_payments_path
          expect(page).to have_text("Requirements")
          expect(page).to have_text("Having trouble? See acceptable business registration documents here.")
          expect(page).to have_link("here", href: "https://docs.stripe.com/acceptable-verification-documents?document-type=entity#select-a-country-to-view-its-requirements")

          page.attach_file("Upload document", Rails.root.join("spec/support/fixtures/success.png"), visible: false)
          expect(page).to have_content("Uploading...")
          expect(page).to have_content("success.png")
          expect(page).to have_content("Thanks! You're all set.")
          expect(page).not_to have_text("Requirements")
      ...
      it "allows the singapore creator to perform enhanced identity verification" do
        user = create(:user)
        create(:user_compliance_info_singapore, user:, birthday: Date.new(1901, 1, 2))
        create(:singaporean_bank_account, user:)
        create(:tos_agreement, user:)
        ...
        expect(user.user_compliance_info_requests.requested.
          where(field_needed: UserComplianceInfoFields::Individual::STRIPE_ENHANCED_IDENTITY_VERIFICATION).count).to eq(1)

        login_as user
        visit settings_payments_path
        expect(page).to have_text("In order to comply with Singapore regulations we require additional enhanced identity verification.")
        expect(page).to have_button("Verify my identity")
      end
    ...
    describe "CA corporation requiring company registration verification document" do
      ...
      it "allows to enter bank account details" do
        ...
        visit settings_payments_path
        expect(page).to have_text("We need a document to verify company registration")

        page.attach_file("Upload image", Rails.root.join("spec/support/fixtures/test.pdf"), visible: false)
        expect(page).to have_content("Uploading...")
        expect(page).to have_content("test.pdf")
        expect(page).to have_content("Thanks! You're all set.")
        expect(page).not_to have_text("We need a document to verify company registration")
      end
    end
    ...
    describe "AE business" do
      ...
      it "allows the creator to enter their business vat number and updates it on stripe connect account" do
        ...
        login_as user
        visit settings_payments_path
        expect(page).to have_text("Your business VAT ID number")

        fill_in("Your business VAT ID number", with: "000000000000000")
        click_on("Submit")

        expect(page).to have_content("Thanks! You're all set")
        expect(page).not_to have_text("Your business VAT ID number")
      end

      it "allows the creator to upload their passport scan and updates it on stripe connect account" do
        user = create(:user, username: nil, payment_address: nil)
        create(:user_compliance_info_uae_business, user:, birthday: Date.new(1901, 1, 2))
        create(:uae_bank_account, user:)
        create(:tos_agreement, user:)
        ...
        create(:user_compliance_info_request, user:, field_needed: UserComplianceInfoFields::Individual::PASSPORT)

        login_as user
        visit settings_payments_path
        expect(page).to have_text("We need your passport")

        page.attach_file("Upload image", Rails.root.join("spec/support/fixtures/test.pdf"), visible: false)
        expect(page).to have_content("Uploading...")
        expect(page).to have_content("test.pdf")
        expect(page).to have_content("Thanks! You're all set.")
        expect(page).not_to have_text("We need your passport")
      end

      it "allows the creator to upload their visa scan and updates it on stripe connect account" do
        user = create(:user, username: nil, payment_address: nil)
        create(:user_compliance_info_uae_business, user:, birthday: Date.new(1901, 1, 2))
        create(:uae_bank_account, user:)
        create(:tos_agreement, user:)
        ...
        create(:user_compliance_info_request, user:, field_needed: UserComplianceInfoFields::Individual::VISA)

        login_as user
        visit settings_payments_path
        expect(page).to have_text("We need your visa")

        page.attach_file("Upload image", Rails.root.join("spec/support/fixtures/success.png"), visible: false)
        expect(page).to have_content("Uploading...")
        expect(page).to have_content("success.png")
        expect(page).to have_content("Thanks! You're all set.")
        expect(page).not_to have_text("We need your visa")
      end

      it "allows the creator to upload their bank statement scan and updates it on stripe connect account" do
        user = create(:user, username: nil, payment_address: nil)
        create(:user_compliance_info_uae_business, user:, birthday: Date.new(1901, 1, 2))
        create(:uae_bank_account, user:)
        create(:tos_agreement, user:)
        ...
        create(:user_compliance_info_request, user:, field_needed: UserComplianceInfoFields::Business::BANK_STATEMENT)

        login_as user
        visit settings_payments_path
        expect(page).to have_text("We need a proof of active bank account such as a bank account statement")

        page.attach_file("Upload image", Rails.root.join("spec/support/fixtures/success.png"), visible: false)
        expect(page).to have_content("Uploading...")
        expect(page).to have_content("success.png")
        expect(page).to have_content("Thanks! You're all set.")
        expect(page).not_to have_text("We need a proof of active bank account such as a bank account statement")
      end

      it "allows the creator to upload their memorandum of association scan and updates it on stripe connect account" do
        user = create(:user, username: nil, payment_address: nil)
        create(:user_compliance_info_uae_business, user:, birthday: Date.new(1901, 1, 2))
        create(:uae_bank_account, user:)
        create(:tos_agreement, user:)
        ...
        create(:user_compliance_info_request, user:, field_needed: UserComplianceInfoFields::Business::MEMORANDUM_OF_ASSOCIATION)

        login_as user
        visit settings_payments_path
        expect(page).to have_text("We need your memorandum of association (or equivalent document)")

        page.attach_file("Upload image", Rails.root.join("spec/support/fixtures/success.png"), visible: false)
        expect(page).to have_content("Uploading...")
        expect(page).to have_content("success.png")
        expect(page).to have_content("Thanks! You're all set.")
        expect(page).not_to have_text("We need your memorandum of association (or equivalent document)")
      end

      it "allows the creator to upload their power of attorney scan and updates it on stripe connect account" do
        user = create(:user, username: nil, payment_address: nil)
        create(:user_compliance_info_uae_business, user:, birthday: Date.new(1901, 1, 2))
        create(:uae_bank_account, user:)
        create(:tos_agreement, user:)
        ...
        create(:user_compliance_info_request, user:, field_needed: UserComplianceInfoFields::Individual::POWER_OF_ATTORNEY)

        login_as user
        visit settings_payments_path
        expect(page).to have_text("We need a Power of Attorney document for the company representative")

        page.attach_file("Upload image", Rails.root.join("spec/support/fixtures/success.png"), visible: false)
        expect(page).to have_content("Uploading...")
        expect(page).to have_content("success.png")
        expect(page).to have_content("Thanks! You're all set.")
        expect(page).not_to have_text("We need a Power of Attorney document for the company representative")
      end
    end
