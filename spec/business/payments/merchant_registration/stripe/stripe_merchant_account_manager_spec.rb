  ...
  describe ".handle_stripe_event" do
    ...
    describe "event: account.updated" do
      ...
      describe "for a USA creator" do
        ...
        describe "charges_enabled status" do
          describe "in the request charges are newly disabled" do
            ...
            it "emails the creator if charges_enabled was previously true" do
              stripe_event["data"]["previous_attributes"] = { "charges_enabled" => true }
            ...
            it "does not email the creator if charges_enabled was previously not set" do
              expect do
                described_class.handle_stripe_event(stripe_event)
              end.not_to have_enqueued_mail(MerchantRegistrationMailer, :stripe_charges_disabled).with(user.id)
            end
        ...
        describe "payouts_enabled status" do
          describe "in the request payouts are disabled" do
            ...
            it "pauses the payouts on the account and notifies the creator by email" do
      ...
      describe "requests for information including some unrecognized fields" do
        ...
        it "creates a user compliance info request record for the supported fields, emails the creator, and logs an error about unsupported fields" do
          expect(Rails.logger).to receive(:error).with(/Stripe requested unrecognized fields \(\["individual.verification.unrecognized_field"\]\) for account ac1 in event stripe-event-id-1!/).and_call_original
          expect(Bugsnag).to receive(:notify).with(/Stripe requested unrecognized fields \(\["individual.verification.unrecognized_field"\]\) for account ac1 in event stripe-event-id-1!/)

          expect do
            described_class.handle_stripe_event(stripe_event_1)
          end.to have_enqueued_mail(ContactingCreatorMailer, :more_kyc_needed).with(user.id, %w(stripe_additional_document_id stripe_identity_document_id))

          expect(UserComplianceInfoRequest.count).to eq 2
          expect(UserComplianceInfoRequest.first.field_needed).to eq(UserComplianceInfoFields::Individual::STRIPE_ADDITIONAL_DOCUMENT_ID)
          expect(UserComplianceInfoRequest.last.field_needed).to eq(UserComplianceInfoFields::Individual::STRIPE_IDENTITY_DOCUMENT_ID)
        end
      end
      ...
      describe "risk-related information updates" do
        ...
        let(:stripe_event) do
          {
            ...
            "data" => {
              "object" => {
                ...
                "business_type" => "individual",
                "requirements" => {
                  "currently_due" => ["interv_cmVxbXRfMVEyOTViUzhuV09PRjdyT0ZPamtGelgxv1000c65GRfs.supportability.intellectual_property_usage.form"],
                  "eventually_due" => [],
                  "past_due" => [],
                  "current_deadline" => 1712086848,
                },
        ...
        it "adds user compliance info request if supportability fields are due and sends remediation email to the creator" do
          ...
          expect(user_compliance_info_request.field_needed).to eq("interv_cmVxbXRfMVEyOTViUzhuV09PRjdyT0ZPamtGelgxv1000c65GRfs.supportability.intellectual_property_usage.form")
          expect(user_compliance_info_request.only_needs_field_to_be_partially_provided).to be false
        ...
        it "does not add user compliance info request if compliance fields are due and does not send remediation email to the creator" do
          stripe_event.deep_merge!("data" => {
                                     "object" => {
                                       "requirements" => {
                                         "currently_due" => ["interv_cmVxbXRfMVEyOTViUzhuV09PRjdyT0ZPamtGelgxv1000c65GRfs.compliance.identity_verification.challenge"],
                                       }
                                     }
                                   })
          ...
          expect do
            expect do
              described_class.handle_stripe_event(stripe_event)
            end.not_to have_enqueued_mail(ContactingCreatorMailer, :stripe_remediation).with(user.id)
          end.not_to change(UserComplianceInfoRequest, :count)
        end
        ...
        it "adds user compliance info request if credit-related fields are due and sends remediation email to the creator" do
          stripe_event.deep_merge!("data" => {
                                     "object" => {
                                       "requirements" => {
                                         "currently_due" => ["interv_cmVxbXRfMVEyOTViUzhuV09PRjdyT0ZPamtGelgxv1000c65GRfs.credit.review.form"],
                                       }
                                     }
                                   })
          ...
          expect(user_compliance_info_request.user).to eq(user)
          expect(user_compliance_info_request.state).to eq("requested")
          expect(user_compliance_info_request.field_needed).to eq("interv_cmVxbXRfMVEyOTViUzhuV09PRjdyT0ZPamtGelgxv1000c65GRfs.credit.review.form")
          expect(user_compliance_info_request.only_needs_field_to_be_partially_provided).to be false
          expect(user_compliance_info_request.due_at).to eq(Time.zone.at(1712086848))
          expect(user_compliance_info_request.stripe_event_id).to eq("stripe-event-id-1")
        end

        it "clears user name, bio, and all products and triggers stripe re-review if stripe has rejected account due to compliance issue" do
          expect(StripeMerchantAccountManager).to receive(:update_account)
          ...
          create(:product, user: user)
          expect(user.reload.products.alive.count).to eq(2)
          expect(user.products.alive.is_unpublished_by_admin.count).to eq(0)

          stripe_event.deep_merge!("data" => {
                                     "object" => {
                                       "requirements" => {
                                         "currently_due" => ["interv_cmVxbXRfMVEyOTViUzhuV09PRjdyT0ZPamtGelgxv1000c65GRfs.compliance.rejection_appeal.support"],
                                       }
                                     }
                                   })
          ...
          expect(user.flagged_for_tos_violation?).to be true
          expect(user.tos_violation_reason).to eq("Stripe reported terms violation")
          expect(user.reload.products.alive.count).to eq(0)
          expect(user.reload.products.is_unpublished_by_admin.count).to eq(2)
        ...
        it "suspends the account and notifies the user via email if stripe has rejected account due to supportability issue" do
          stripe_event.deep_merge!("data" => {
                                     "object" => {
                                       "requirements" => {
                                         "currently_due" => ["interv_cmVxbXRfMVEyOTViUzhuV09PRjdyT0ZPamtGelgxv1000c65GRfs.supportability.rejection_appeal.support"],
                                       }
                                     }
                                   })
        ...
        it "suspends the account and notifies the user via email if stripe has rejected account due to credit-related issue" do
          stripe_event.deep_merge!("data" => {
                                     "object" => {
                                       "requirements" => {
                                         "currently_due" => ["interv_cmVxbXRfMVEyOTViUzhuV09PRjdyT0ZPamtGelgxv1000c65GRfs.credit.rejection_appeal.support"],
                                       }
                                     }
                                   })
        ...
        it "does not do anything if Stripe notifies about a reserve placed on the Stripe account" do
          stripe_event.deep_merge!("data" => {
                                     "object" => {
                                       "requirements" => {
                                         "currently_due" => ["interv_cmVxbXRfMVEyOTViUzhuV09PRjdyT0ZPamtGelgxv1000c65GRfs.credit.reserve_appeal.form"],
                                       }
                                     }
                                   })
          ...
          expect do
            expect do
              described_class.handle_stripe_event(stripe_event)
            end.not_to have_enqueued_mail(ContactingCreatorMailer, :suspended_due_to_stripe_risk).with(user.id)
          end.not_to have_enqueued_mail(ContactingCreatorMailer, :stripe_remediation).with(user.id)
