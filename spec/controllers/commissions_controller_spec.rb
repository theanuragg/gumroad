  ...
  describe "POST complete" do
    ...
    context "when an error occurs during completion" do
      it "returns an error message" do
        allow_any_instance_of(Commission).to receive(:create_completion_purchase!).and_raise(StandardError.new("Completion failed"))

        post :complete, params: { id: commission.external_id }

        expect(response).to have_http_status(:unprocessable_entity)
        expect(response.parsed_body).to eq({ "errors" => ["Completion failed"] })
      end
    end
