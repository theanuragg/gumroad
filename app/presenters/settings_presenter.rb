...
class SettingsPresenter
  ...
  def payments_props(remote_ip: nil)
    user_compliance_info = seller.fetch_or_build_user_compliance_info
    {
      ...
      verification_details: verification_details(user_compliance_info),
      paypal_connect:,
      ...
      stripe_connect_embedded_notification_banner_enabled: Feature.active?(:stripe_connect_embedded_notification_banner, seller),
    }
  end
  ...
  private
    ...
    def verification_details(user_compliance_info)
      fields_needed = seller.user_compliance_info_requests.requested.pluck(:field_needed)

      verification_errors = {}
      UserComplianceInfoFields::VERIFICATION_PROMPT_FIELDS.each do |field|
        request = seller.user_compliance_info_requests.requested.where(field_needed: field).last
        verification_errors[field.to_sym] = request&.verification_error_message
      end

      prompt_for_full_ssn = user_compliance_info.country_code == Compliance::Countries::USA.alpha2 &&
        seller.user_compliance_info_requests
              .requested
              .where(field_needed: UserComplianceInfoFields::Individual::TAX_ID)
              .not_only_needs_field_to_be_partially_provided
              .exists?

      prompt_for_business_vat_number = user_compliance_info.country_code == Compliance::Countries::ARE.alpha2 &&
        UserComplianceInfoFields::Business::VAT_NUMBER.in?(fields_needed)
      prompt_for_photo_id = UserComplianceInfoFields::Individual::STRIPE_IDENTITY_DOCUMENT_ID.in?(fields_needed)
      prompt_for_additional_id = UserComplianceInfoFields::Individual::STRIPE_ADDITIONAL_DOCUMENT_ID.in?(fields_needed)
      prompt_for_company_id = UserComplianceInfoFields::Business::STRIPE_COMPANY_DOCUMENT_ID.in?(fields_needed)
      prompt_for_passport = UserComplianceInfoFields::Individual::PASSPORT.in?(fields_needed)
      prompt_for_visa = UserComplianceInfoFields::Individual::VISA.in?(fields_needed)
      prompt_for_power_of_attorney = UserComplianceInfoFields::Individual::POWER_OF_ATTORNEY.in?(fields_needed)
      prompt_for_memorandum_of_association = UserComplianceInfoFields::Business::MEMORANDUM_OF_ASSOCIATION.in?(fields_needed)
      prompt_for_proof_of_registration = UserComplianceInfoFields::Business::PROOF_OF_REGISTRATION.in?(fields_needed)
      prompt_for_company_registration_verification = UserComplianceInfoFields::Business::COMPANY_REGISTRATION_VERIFICATION.in?(fields_needed)
      prompt_for_bank_statement = UserComplianceInfoFields::Business::BANK_STATEMENT.in?(fields_needed)
      prompt_for_enhanced_identity_verification = UserComplianceInfoFields::Individual::STRIPE_ENHANCED_IDENTITY_VERIFICATION.in?(fields_needed)

      show_verification_section = (prompt_for_full_ssn || prompt_for_photo_id || prompt_for_additional_id ||
        prompt_for_company_id || prompt_for_business_vat_number || prompt_for_passport || prompt_for_visa ||
        prompt_for_power_of_attorney || prompt_for_memorandum_of_association || prompt_for_bank_statement ||
        prompt_for_proof_of_registration || prompt_for_company_registration_verification ||
        prompt_for_enhanced_identity_verification) && seller.stripe_account.present? && Pundit.policy!(pundit_user, [:settings, :payments, seller]).update?

      {
        ...
        verification_errors:,
        country: user_compliance_info.legal_entity_country,
      }
    end
    ...
    def paypal_connect
      ...
      stripe_connect_account = seller.merchant_migration_enabled? && seller.stripe_connect_account
...

      {
        ...
        is_native_payout_set_up: seller.payment_address.present? || (stripe_connect_account.present? || seller.active_bank_account.present?) && seller.native_payouts_supported?,
