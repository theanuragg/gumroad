...
export const Layout = ({ title, headerActions, children }: LayoutProps) => (
  <main>
    <header>
      <h1>{title}</h1>
...
const CollaboratorDetails = ({
  ...
  const resendInvite = () => console.log(`TODO - resend invite to ${selectedCollaborator.id}`);
    ...
    <aside>
      ...
      <section className="stack">
        ...
        <div>
          ...
          {selectedCollaborator.setup_incomplete ? <a onClick={resendInvite}>Resend invite</a> : null}
        </div>
      </section>
      ...
      <section className="stack">
        ...
        <section>
          <Link
            ...
            className={cx("button", { primary: selectedCollaborator.products.length === 0 })}
            aria-label="Edit"
            inert={!loggedInUser?.policies.collaborator.update || navigation.state !== "idle"}
          >
            {selectedCollaborator.products.length === 0 ? "Set products" : "Edit products"}
          </Link>
        </section>
      </section>

      <section style={{ display: "flex", gap: "var(--spacer-4)" }}>
        <Link
          style={{ flex: 1 }}
          to={`/collaborators/${selectedCollaborator.id}/edit`}
          className="button"
...
const Collaborators = () => {
  ...
  const { collaborators, collaborators_disabled_reason } = cast<CollaboratorsData>(useLoaderData());
              ...
              <tbody>
                {collaborators.map((collaborator) => (
                  ...
                  >
                    ...
                    <td data-label="Products">{formatProductNames(collaborator)}</td>
                    <td data-label="Cut">{formatCommission(collaborator)}</td>
                    <td>
