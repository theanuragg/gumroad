...
const WithContent = ({
  ...
  const unprocessedAudioIds =
    content.rich_content_pages !== null && props.is_mobile_app_web_view
      ...
      : [];
  const missingAudioDurationsFetchIntevalRef = React.useRef<number>();
  React.useEffect(() => {
  ...
  useRunOnce(() => {
    ...
    if (content.rich_content_pages != null && contentFiles.length > 0) {
      setInterval(fetchLatestMediaLocations, LATEST_MEDIA_LOCATIONS_FETCH_INTERVAL_IN_MS);
    }
  });
