...
class Balance < ApplicationRecord
  ...
  %w[unpaid processing paid forfeited].each do |paid_state|
    scope(paid_state.to_sym, -> { where(state: paid_state) })
  end

  def self.create!(user:, merchant_account:, currency:, holding_currency:, date:)
    balance = new
    balance.user = user
    balance.merchant_account = merchant_account
    balance.currency = currency
    balance.holding_currency = holding_currency
    balance.date = date
    balance.save!
    balance
  end
