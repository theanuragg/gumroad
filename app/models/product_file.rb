...
class ProductFile < ApplicationRecord
  include S3Retrievable, WithFileProperties, ExternalId, SignedUrlHelper, JsonData, SendableToKindle, Deletable,
          CdnDeletable, FlagShihTzu, CdnUrlHelper
  ...
  def transcodable?
    # Can't transcode if the ProductFile is not streamable. Can't transcode if the video hasn't been analyzed yet.
  ...
  # Public: hls_playlist
  #
  # This method returns a string representing the secured m3u8 HLS playlist content for this product file.
  #
  # This method can return nil if there are no transcoded videos for the product file.
  #
  def hls_playlist
    ...
    playlist.split("\n").each do |playlist_line|
      if playlist_line.start_with?("#")
        ...
        # Escape the user-provided portion of the url, which is the original file name. Note that the ProductFile could have been
        # renamed by now. That's why we get the original file name from the hls_key_prefix, which comes from the TranscodedVideo object that was created
        # when the video was originally transcoded.
        ...
        resolution_specific_playlist_key.gsub!(original_file_name, CGI.escape(original_file_name))

        signed_resolution_specific_playlist_url = signed_cloudfront_url("#{HLS_DISTRIBUTION_URL}#{resolution_specific_playlist_key}", is_video: true)
  ...
  def schedule_file_analyze
    AnalyzeFileWorker.perform_in(5.seconds, id) unless external_link?
  end
