...
class Purchase < ApplicationRecord
 ...
 "(purchases.chargeback_date IS NOT NULL AND purchases.flags & ? != 0)", Purchase.flag_mapping["flags"][:chargeback_reversed])
  ...
  def offer_amount_off(purchase_min_price)
    original_offer_code.present? ? original_offer_code.amount_off(purchase_min_price) : 0
  end
  ...
  private
    ...
    def variants_satisfied
      return if is_preorder_charge? || is_recurring_subscription_charge || link.native_type == Link::NATIVE_TYPE_COFFEE
