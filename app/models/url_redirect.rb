...
class UrlRedirect < ApplicationRecord
  ...
  def update_transcoded_videos_last_accessed_at
    ...
    now = Time.current
    videos_ids.each_slice(1_000) do
      TranscodedVideo.alive.where(product_file: _1).update_all(last_accessed_at: now)
    end
  end
  ...
  def enqueue_job_to_regenerate_deleted_transcoded_videos
    ...
    alive = videos_ids.each_slice(1_000).map { TranscodedVideo.alive.where(product_file: _1).pluck(:product_file_id) }.flatten
    missing = (videos_ids - alive).uniq

    missing_because_deleted = TranscodedVideo.deleted.completed.where(product_file: missing).includes(:product_file).map(&:product_file).uniq.select(&:transcodable?)

    missing_because_deleted.each_with_index do |product_file, i|
      delay = 5.minutes * i
      TranscodeVideoForStreamingWorker.perform_in(delay, product_file.id)
    end
  end
