...
class Link < ApplicationRecord
  ...
  has_flags 1 => :product_refund_policy_enabled,
            ...
            30 => :DEPRECATED_sales_require_confirmed_buyer_account,
  ...
  has_many :direct_affiliates, -> { direct }, through: :product_affiliates, source: :affiliate
  has_many :global_affiliates, -> { global }, through: :product_affiliates, source: :affiliate
  has_many :collaborators, -> { collaborator }, through: :product_affiliates, source: :affiliate
  ...
  scope :for_collaborating_user, ->(user) do
    Link.joins(product_affiliates: :affiliate)
      .is_collab
      .where(affiliates: { affiliate_user: user })
      .merge(Collaborator.alive)
  end
  scope :collab_for_user, ->(user) do
    as_seller = Link.is_collab.where(user:)
    as_collaborator = Link.for_collaborating_user(user)

    subquery_sqls = [as_seller, as_collaborator].map(&:to_sql)
    from("(" + subquery_sqls.join(" UNION ") + ") AS #{table_name}")
  end
  scope :for_balance_page, ->(user) do
    products_as_seller = Link.where(user: user)
    collabs_as_collaborator = Link.for_collaborating_user(user)
  ...
  def collaborator
    collaborators.alive.take
  end

  def collaborator_for_display
    collaborator.affiliate_user if collaborator&.show_as_co_creator_for_product?(self)
  end

  def percentage_revenue_cut_for_user(user)
    is_seller = user.id == user_id
    return is_seller ? 100 : 0 unless is_collab?

    collaborator_percentage_cut = product_affiliates.find_by(affiliate: collaborator).affiliate_percentage
    if is_seller
      100 - collaborator_percentage_cut
    elsif user.id == collaborator.affiliate_user_id
  ...
  def has_another_collaborator?(collaborator: nil)
    live_collaborators = collaborators.alive
    live_collaborators = live_collaborators.where.not(id: collaborator.id) if collaborator.present?
    live_collaborators.exists?
  end
