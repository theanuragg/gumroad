...
class TranscodeVideoForStreamingWorker
  ...
  def perform(id, klass_name = ProductFile.name, transcoder = GRMC, allowed_when_processing = false)
    ...
    return if product_file.deleted?

    return unless product_file.attempt_to_transcode?(allowed_when_processing:)
    product_file.transcoded_videos.alive.processing.find_each(&:mark_error) if allowed_when_processing

    if product_file.transcodable?
      create_hls_transcode_job(product_file, product_file.s3_key, product_file.height, transcoder)
    else
  ...
  def create_hls_transcode_job(product_file, original_video_key, input_video_height, transcoder)
    if mediaconvert_transcodable?(original_video_key, transcoder)
      create_hls_mediaconvert_transcode_job(product_file, original_video_key, input_video_height, transcoder)
    else
      create_hls_ets_transcode_job(product_file, original_video_key, input_video_height)
    end
  end

  def create_hls_ets_transcode_job(product_file, original_video_key, input_video_height)
    input = {
  ...
  def create_hls_mediaconvert_transcode_job(product_file, original_video_key, input_video_height, transcoder)
    # Every video gets transcoded to 480p regardless of its height. HD Videos get transcoded to at most one more preset depending on their resolution:
    presets = ["hls_480p"]
    ...
    # Attempt to use GRMC if possible, otherwise fallback to AWS MediaConvert
    if transcoder == GRMC && Feature.active?(:grmc)
      begin
        ...
        if response.code == "200"
          ...
          TranscodeVideoForStreamingWorker.perform_in(24.hours, product_file.id, ProductFile.name, MEDIACONVERT, true)

          return
        elsif response.code == "429"
          Rails.logger.warn("GRMC is busy (product_file_id: #{product_file.id})")
        else
          Rails.logger.warn("Failed request to GRMC (product_file_id: #{product_file.id}): #{response.code} => #{response.body}")
        end
      rescue => e
  ...
  private
    def mediaconvert_transcodable?(original_video_key, transcoder)
      return false unless transcoder.in?([MEDIACONVERT, GRMC])
