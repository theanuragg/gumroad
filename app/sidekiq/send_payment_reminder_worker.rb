...
class SendPaymentReminderWorker
  ...
  def perform
    User.payment_reminder_risk_state.announcement_notification_enabled
        ...
        .find_each do |user|
      ContactingCreatorMailer.remind(user.id).deliver_later(queue: "low") if user.active_bank_account.nil? && user.stripe_connect_account.blank? && user.form_email.present?
    end
  end
end
