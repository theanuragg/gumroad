...
module WithFileProperties
  ...
  def link_published?
    (respond_to?(:published?) && published?) || link.try(:alive?)
  end

  def installment_published?
    (respond_to?(:published?) && published?) || installment.try(:published?)
  end
  ...
  def assign_video_attributes(path)
    ...
    if installment_published?
      transcode_video(id)
    elsif link_published?
  ...
  private
    ...
    def transcode_video(id, class_name = self.class.name)
      TranscodeVideoForStreamingWorker.perform_in(10.seconds, id, class_name)
    end
end
