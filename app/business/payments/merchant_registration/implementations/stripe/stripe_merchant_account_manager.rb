...
module StripeMerchantAccountManager
  ...
  def self.handle_stripe_info_requirements(stripe_event_id, stripe_account, stripe_previous_attributes)
    ...
    stripe_fields_needed.each do |stripe_field_needed|
      field_needed = StripeUserComplianceInfoFieldMap.map(stripe_field_needed)
      if field_needed
      ...
      elsif stripe_field_needed.match?(/^interv_/)
        stripe_risk_fields_needed << stripe_field_needed
      else
        unrecognized_fields << stripe_field_needed
      end
    end
    ...
    stripe_risk_fields_needed.each do |stripe_risk_field_needed|
      ...
      risk_requirement_sub_category = stripe_risk_field_needed.split(".")[2]
      risk_requirement_resolution_path = stripe_risk_field_needed.split(".")[3]

      if risk_requirement_sub_category == "rejection_appeal"
        case risk_requirement_category
        when "supportability"
        ...
        when "credit"
          # Account not supportable due to presenting a higher level of risk.
          # Suspend the account and inform the creator via email.
          user.suspend_due_to_stripe_risk
        when "compliance"
          # Account not supportable under Stripe ToS.
          # Delete all products, clear bio and name, and update on Stripe for re-review.
          user.update!(tos_violation_reason: "Stripe reported terms violation")
          user.flag_for_tos_violation!(author_name: "stripe_risk", bulk: true, content: "All products were unpublished because Stripe reported terms violation") unless user.flagged? || user.suspended? || user.on_probation?
          user.bio = "" if AdultKeywordDetector.adult?(user.bio)
          user.name = "" if AdultKeywordDetector.adult?(user.name)
          user.username = nil if AdultKeywordDetector.adult?(user.username)
          if user.changed?
            user.save!
            GenerateUsernameJob.perform_async(user.id)
          end
          user.links.alive.find_each do |product|
            product.unpublish!(is_unpublished_by_admin: true)
          end
          handle_new_user_compliance_info(user.alive_user_compliance_info)
        else
          unrecognized_fields << stripe_risk_field_needed
        end
      elsif %w(form challenge).include?(risk_requirement_resolution_path) &&
        %w(supportability credit).include?(risk_requirement_category)
        next if risk_requirement_sub_category == "reserve_appeal"
        ...
        ContactingCreatorMailer.stripe_remediation(user.id).deliver_later
      elsif risk_requirement_category == "platform_concern"
        # These are requirements raised by the platform i.e. Gumroad.
        # Since Gumroad does not raise any info requirements via Stripe, we do not expect to receive these.
        unrecognized_fields << stripe_risk_field_needed
      else
        # Unknown requirement received from Stripe.
        unrecognized_fields << stripe_risk_field_needed
      end
    end
    ...
    merchant_account.user.user_compliance_info_requests.requested.find_each do |user_compliance_info|
    ...
    MerchantRegistrationMailer.stripe_charges_disabled(user.id).deliver_later(queue: "critical") if charges_newly_disabled && !user.suspended? && !merchant_account.deleted?
    ...
    if unrecognized_fields.any?
      error_message = "Stripe requested unrecognized fields (#{unrecognized_fields.inspect}) for account #{stripe_account_id} in event #{stripe_event_id}!"
    ...
    all_fields_needed = user.user_compliance_info_requests.requested.map(&:field_needed).uniq
